// EditProductScreen.tsx — Экран редактирования продукта
...
import { Room, Unit, ROOMS, UNITS, Category, CATEGORIES, InventoryItem } from '../models';
import { updateInventoryItem, getInventory } from '../services/storage';
...
export default function EditProductScreen() {
  const route = useRoute<RouteProp<EditProductRouteParams, 'EditProduct'>>();
  ...
  const loadProduct = async () => {
    try {
      ...
      if (product) {
        ...
        setExpiryDays(Math.max(1, days).toString());
      }
    } catch (error) {
      Alert.alert('Ошибка', 'Не удалось загрузить продукт');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Успешно', 'Продукт обновлён!', [
